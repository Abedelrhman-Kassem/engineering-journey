...
**Configuration will enter at the composition root.** `Host` owns `appsettings.json`, so when a layer first needs a setting, `Host` hands `IConfiguration` to that layer's registration method and the layer reads what it needs once, while it is being wired. That parameter was written and then taken out again: nothing in `Infrastructure` reads a setting today, and a parameter nobody reads is the same defect as a registration method that registers nothing — one minute to add back on the day it earns its place. Whenever it returns, the rule around it holds: services must not take `IConfiguration` as a constructor dependency, because a misspelled key would surface at the first call rather than at startup, and nothing about the class would declare which settings it depends on. Strongly-typed options with validate-on-start replace the whole approach in Task 2.6, which may well arrive before the first setting does.
